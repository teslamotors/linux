...
``V4L2_CID_MPEG_VIDEO_VP9_P_FRAME_QP``
    Quantization parameter for an P frame for VP9.
...
``V4L2_CID_MPEG_VIDEO_VP9_RC_FRAME_RATE``
    Selects the RC filter frame rate for VP9 encoder.

``V4L2_CID_MPEG_VIDEO_VP9_GOLDEN_FRAMESEL``
    Indicates the golden frame. Valid when NUM_OF_REF is 2.
    Zero indicates use the previous second frame as a golden frame.
    One indicates use the previous specific frame indicated by
    GOLDEN_FRAME_REFRESH_PERIOD as a golden frame.
...
``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHY_QP_ENABLE``
    Allows host to specify the QP values for each temporal layer through
    HEIRARCHIAL_QP_LAYER. This is valid only if HIERARCHIAL_CODING_LAYER
    is greater than 1.

``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER_QP``
    Indicates the quantization parameter of the layer for VP9 encoder.

``V4L2_CID_MPEG_VIDEO_VP9_REF_NUMBER_FOR_PFRAMES``
    Number of reference pictures for encoding P frames. In case of 0 frame of
    VP9, last encoded frame will be searched, In case of 1 frame of VP9, last
    encoded frame and golden frame will be searched. The golden frame is
    determined by  GOLDEN_FRAME_SEL. Zero indicates 1 reference frame.
    One indicates 2 reference frame.
...
``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER``
    ...
    VP9 has uoto 3 layer of encoder.

``V4L2_CID_MPEG_VIDEO_VP9_MAX_PARTITION_DEPTH``
    Indicates maximum coding unit depth. Zero indicates no CU partition dept.
    One indicates allow one CU partition depth.
...
``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER_BIT0``
    Indicates hierarchical coding layer BIT0 for HEVC encoder.

``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER_BIT1``
    Indicates hierarchical coding layer BIT1 for VP9 encoder.

``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER_BIT2``
    Indicates hierarchical coding layer BIT2 for VP9 encoder.

``V4L2_CID_MPEG_VIDEO_VP9_HIERARCHICAL_CODING_LAYER_CH``
    Indicates hierarchical coding layer change for VP9 encoder.
