			...
			uint8_t *wrapped_key)
{
	...
	commandId = DAL_KEYSTORE_GENERATE_KEY;
	out_buf = kmalloc(output_len, GFP_KERNEL);
	if (!out_buf)
		return -ENOMEM;
...
int dal_keystore_unload_key(const uint8_t *client_ticket, unsigned int slot_id)
{
	...
	commandId = DAL_KEYSTORE_UNLOAD_KEY;
	memcpy(input, client_id, sizeof(client_id));
	index += sizeof(client_id);
	res = pack_int_to_buf(slot_id, input + sizeof(client_id));
	index += 2;

	if (res) {
		ks_err(KBUILD_MODNAME ": %s Error: slot id is too big%d\n",
		__func__, res);
		return res;
	}
	out_buf = kmalloc(output_len, GFP_KERNEL);
			...
			uint8_t *output)
{
	...
	int commandId = 0;
	/* iv size 16 */
	uint8_t in[KEYSTORE_MAX_CLIENT_ID_SIZE + DAL_KEYSTORE_GCM_IV_SIZE
			+ input_size + 6];
	size_t index = 0;
	size_t output_len = 0;
	size_t response_code = 0;
	...
	memcpy(in, client_id, sizeof(client_id));
	index += sizeof(client_id);
	res = pack_int_to_buf(slot_id, (uint8_t *)(in + index));
	index += 2;
	...
	commandId = DAL_KEYSTORE_ENCRYPT;
	out_buf = kmalloc(output_len, GFP_KERNEL);
	if (!out_buf)
		return -ENOMEM;
			...
			uint8_t *output)
{
	...
	commandId = DAL_KEYSTORE_DECRYPT;
	out_buf = kmalloc(output_len, GFP_KERNEL);
	if (!out_buf)
		return -ENOMEM;
