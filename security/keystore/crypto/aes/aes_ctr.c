 ...
 */
int keystore_aes_ctr_crypt(int enc, const char *key, size_t klen, const char *iv
		, size_t ivlen,	const char *data_in, char *data_out, size_t len)
{
...
#ifdef DEBUG_TRACE
	ks_info(KBUILD_MODNAME ": keystore_aes_ctr_crypt enc=%d klen=%zu ivlen=%zu len=%zu\n",
			enc, klen, ivlen, len);
#endif

	tfm = crypto_alloc_blkcipher("ctr(aes)", 0, 0);
	if (IS_ERR(tfm)) {
		ks_err(KBUILD_MODNAME ": failed to load transform for ctr(aes): %ld\n",
		       PTR_ERR(tfm));
		rc = PTR_ERR(tfm);
		goto err_tfm;
	}
	...
	crypto_blkcipher_set_iv(tfm, iv, ivlen);
