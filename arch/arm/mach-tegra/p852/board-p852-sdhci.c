...
void __init p852_sdhci_init(void)
{
	...
	if (p852_sku_peripherals & P852_SKU_SDHCI_ENABLE)
		...
		{
			sdhci_config = (p852_sdhci_peripherals >> (P852_SDHCI_SHIFT * i));
			cd = i * 3 ; wp = cd + 1; pw = wp + 1;
			if (sdhci_config & P852_SDHCI_ENABLE)
			{
				if(sdhci_config & P852_SDHCI_CD_EN)
				{
					snprintf(gpio_name[cd], count, "sdhci%d_cd",i);
					gpio_request(p852_sdhci_platform_data[i].cd_gpio, gpio_name[cd]);
					tegra_gpio_enable(p852_sdhci_platform_data[i].cd_gpio);
				}

				if(sdhci_config & P852_SDHCI_WP_EN)
				{
					snprintf(gpio_name[wp], count, "sdhci%d_wp",i);
					gpio_request(p852_sdhci_platform_data[i].wp_gpio, gpio_name[wp]);
					tegra_gpio_enable(p852_sdhci_platform_data[i].wp_gpio);
				}

				if(sdhci_config & P852_SDHCI_PW_EN)
				{
					snprintf(gpio_name[pw], count, "sdhci%d_pw",i);
					gpio_request(p852_sdhci_platform_data[i].power_gpio, gpio_name[pw]);
					tegra_gpio_enable(p852_sdhci_platform_data[i].power_gpio);
				}

				platform_device_register(&tegra_sdhci_device[i]);
			}
		}
}
