...
#define TEGRA_SYSTEM_DMA_AVP_CH_NUM		CONFIG_TEGRA_NUM_AVP_APB_CHANNELS
	...
	if (req) {
		if (req->buffer_status == TEGRA_DMA_REQ_BUF_STATUS_EMPTY) {
			...
			if (!is_dma_ping_complete) {
				...
				req->bytes_transferred = req->size;
				req->status = TEGRA_DMA_REQ_SUCCESS;
				/* Stop the DMA only if the next buffer is
				 * is not available. Stopping the DMA will
				 * cause the I2S FIFO to underflow/overflow
				 * and TDM sync loss issues. Continuing the
				 * DMA is as good as repeating the previous
				 * buffer */
