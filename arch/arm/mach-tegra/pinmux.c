...
int set_pinmux(const char *pingrp, const char *setting)
{
	...
	for (i = 0; i < TEGRA_MAX_PINGROUP; i++)
	{
		if(strcasecmp(pingrp, pingroups[i].name) == 0)
		{
			if(strcasecmp(setting,"NORMAL") == 0)
				err = tegra_pinmux_set_tristate(i, TEGRA_TRI_NORMAL);
			else if(strcasecmp(setting,"TRISTATE") == 0)
				err = tegra_pinmux_set_tristate(i, TEGRA_TRI_TRISTATE);
			else
			{
				printk("%s(): Invalid pingroup state, "
	...
	printk("%s(): Invalid pingroup specified\n", __func__);
	return -EINVAL;
}

ssize_t set_pinmux_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	s = buf = (char *)kmalloc(count + 1, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
	...
	*s++ = '\0';
	...
	if((err = set_pinmux(pingrp, setting)) != 0)
		goto error;
