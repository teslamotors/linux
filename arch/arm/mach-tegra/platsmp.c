...
void __init smp_init_cpus(void)
{
	...
	int sku;
	struct skued_core_limit numcore_limit[] = {NUMCORE_LIMITS};

	sku = tegra_sku_id();

	for (i = 0; i < ARRAY_SIZE(numcore_limit); i++)
	{
		if (sku == numcore_limit[i].sku)
		{
			ncores = numcore_limit[i].numcore;
			break;
		}
	}
