...
extern struct snd_soc_dai tegra_tdm_dai[];
...
static struct snd_soc_dai_link tegra_soc_dai[] = {
	{
	 .name = "tdm1",
	 .stream_name = "tdm1",
	 .cpu_dai = &tegra_tdm_dai[0],
	 .codec_dai = &tegra_dummy_codec_stub_all_dais[1],
	 ...
	 },
	{
	 .name = "tdm2",
	 .stream_name = "tdm2",
	 .cpu_dai = &tegra_tdm_dai[1],
	 .codec_dai = &tegra_dummy_codec_stub_all_dais[2],
	 .init = tegra_codec_init,
	 .ops = &tegra_hifi_ops,
	 },
...
static int __init tegra_init(void)
{
	...
	tegra_snd_device = platform_device_alloc("soc-audio", 0);
	if (!tegra_snd_device) {
