...
static int spdif_set_in_sample_rate(struct tegra_spdif_info *info,
		unsigned int sample_rate) {
...
static int tegra_spdif_hw_params(struct snd_pcm_substream *substream,
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		...
		ret = spdif_set_in_sample_rate(info, params_rate(params));
...
static int tegra_spdif_driver_probe(struct platform_device *pdev) {
	...
	/*	info->spdif_clk = clk_get(&pdev->dev, NULL);
	 printk(KERN_ERR "\n****after clk_getand value spdif_clk=%08x",(unsigned int)info->spdif_clk);
	 printk(KERN_ERR "\n****after clk_getand value spdif_clk_rate=%08x",(unsigned int)info->pdata->spdif_clk_rate);
	 //	if (IS_ERR(info->spdif_clk)) {
	 if (!(info->spdif_clk)) {
	 err = PTR_ERR(info->spdif_clk);
	 printk(KERN_ERR "\n****with PTR_ERR(info->spdif_clk)");
	 goto fail_unmap_mem;
	 }
