		...
		u32 tx_mask, u32 rx_mask, int slots, int slot_width)
{
	...
	if (tx_mask)
		adata->tdm_tx_fmt = frm_len;
	if (rx_mask)
		adata->tdm_rx_fmt = frm_len;
	return 0;
}
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S8:
		...
		rtd->divisor = params_channels(params);
		break;
	case SNDRV_PCM_FORMAT_S16_LE:
		rtd->xfer_resolution = 0x02;
		rtd->divisor = params_channels(params) * 2;
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		rtd->xfer_resolution = 0x04;
		rtd->divisor = params_channels(params) * 3;
		break;
	case SNDRV_PCM_FORMAT_S32_LE:
		rtd->xfer_resolution = 0x05;
		rtd->divisor = params_channels(params) * 4;
		break;
	default:
		return -EINVAL;
	}
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		rtd->tdm_fmt = adata->tdm_rx_fmt;
	}
	if (adata->tdm_mode) {
		rtd->tdm_mode = true;
		val = rv_readl(rtd->acp3x_base + reg_val);
		rv_writel(val | 0x2, rtd->acp3x_base + reg_val);
		rv_writel(rtd->tdm_fmt, rtd->acp3x_base + frmt_reg);
	}
	val = rv_readl(rtd->acp3x_base + reg_val);
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	u32 link_pos_reg, link_pos_val, prev_link_pos_val;
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			case I2S_BT_INSTANCE:
				...
				link_pos_reg = mmACP_BT_TX_LINKPOSITIONCNTR;
				break;
			case I2S_SP_INSTANCE:
			default:
				water_val =
				...
				link_pos_reg = mmACP_I2S_TX_LINKPOSITIONCNTR;
			}
		} else {
			switch (rtd->i2s_instance) {
			case I2S_BT_INSTANCE:
				...
				link_pos_reg = mmACP_BT_RX_LINKPOSITIONCNTR;
				break;
			case I2S_SP_INSTANCE:
			default:
				water_val =
				...
				link_pos_reg = mmACP_I2S_RX_LINKPOSITIONCNTR;
			}
		}
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		counter = 1000;
		prev_link_pos_val =
			rv_readl(rtd->acp3x_base + link_pos_reg);
		while (counter-- > 0) {
			link_pos_val =
				rv_readl(rtd->acp3x_base + link_pos_reg);
			if (link_pos_val != prev_link_pos_val &&
			    link_pos_val % divisor == 0)
				break;
			prev_link_pos_val = link_pos_val;
		}

		rv_writel(val, rtd->acp3x_base + reg_val);

		if (counter <= 0) {
			pr_info("prev_link_pos_val :0x%x, link_pos_val: 0x%x\n",
                                 prev_link_pos_val, link_pos_val);
		}
