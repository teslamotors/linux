...
extern int pci_pm_reenable_device(struct pci_dev *pci_dev);
...
struct acp3x_dev_data {
	...
	struct platform_device *pdevPlt[4];
	u32 pme_en;
};
			   ...
			   const struct pci_device_id *pci_id)
{
	...
	case I2S_MODE:
		...
		/* create dummy codec device */
		adata->pdevPlt[2] = platform_device_register_simple("dummy_w5102_sp",
								 0, NULL, 0);
		if (IS_ERR(adata->pdevPlt[2])) {
			dev_err(&pci->dev, "Cannot register dummy_w5102_sp\n");
			ret = PTR_ERR(adata->pdevPlt[2]);
			goto unregister_pdev2;
		}
		break;
	default:
...
unregister_pdev2:
		platform_device_unregister(adata->pdevPlt[2]);
