			   ...
			   char *buf)
{
	...
	return sprintf(buf, "%lld.%09d#%d\n",
			(long long) pps->assert_raw_tu.sec, pps->assert_raw_tu.nsec,
			pps->assert_sequence);
}
static DEVICE_ATTR_RO(assert_raw);

static ssize_t clear_raw_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct pps_device *pps = dev_get_drvdata(dev);
	...
	return sprintf(buf, "%lld.%09d#%d\n",
			(long long) pps->clear_raw_tu.sec, pps->clear_raw_tu.nsec,
			pps->clear_sequence);
}
static DEVICE_ATTR_RO(clear_raw);
