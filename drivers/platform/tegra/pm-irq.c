...
static inline int read_wake_status(u32 *wake_status)
{
	int len = 0;

#ifdef CONFIG_ARCH_TEGRA_2x_SOC
	*wake_status = readl(tegra_pmc_base + PMC_WAKE_STATUS);
	len = 1;
#else
	*wake_status = readl(tegra_pmc_base + PMC_WAKE_STATUS);
	wake_status++;
	*wake_status = readl(tegra_pmc_base + PMC_WAKE2_STATUS);
	len = 2;
#endif
	return len;
}
...
{
	return read_wake_status(wake_status);
}
...
static void tegra_pm_irq_syscore_resume(void)
{
	...
	read_wake_status(wake_status);

	pr_info("Wake Status[31:0]  = 0x%x\n", wake_status[0]);
	tegra_pm_irq_syscore_resume_helper(wake_status[0], 0);
	pr_info("Wake Status[63:32] = 0x%x\n", wake_status[1]);
	tegra_pm_irq_syscore_resume_helper(wake_status[1], 1);
}
