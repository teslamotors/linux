...
#define SYNAPTICS_DSX_I2C_TCH_ADDR 0x20
#define SYNAPTICS_DSX_I2C_BTLDR_ADDR 0x2C
#define SYNAPTICS_DSX_I2C_IRQ_GPIO 452
#define SYNAPTICS_DSX_I2C_RES_GPIO 454
#define SYNAPTICS_MAX_X 2200
#define SYNAPTICS_MAX_Y 1300
	...
	while (remaining_msgs) {
		...
		if (retry == SYN_I2C_RETRY_TIMES) {
			dev_warn(rmi4_data->pdev->dev.parent,
				"%s: I2C read over retry limit of %d\n",
				__func__, retry);
			retval = -EIO;
			goto exit;
		}
		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
		 "%s: Writing %d byte(s) to addr 0x%X\n",
...
#if 0
// CONFIG_ACPI
static const struct acpi_device_id synaptics_rmi4_acpi_match_table [] = {
	{ "SYNAPTICS", 0},
	{ }
};
MODULE_DEVICE_TABLE(acpi, acpi_device_id)
#else
#define synaptics_rmi4_acpi_match_table NULL
#endif
...
static struct synaptics_dsx_button_map _virtual_button_map = {
	.nbuttons = 0,
	.map = NULL
};

static struct synaptics_dsx_button_map _capacitive_button_map = {
	.nbuttons = 0,
	.map = NULL
};
...
static struct synaptics_dsx_board_data _synaptics_platform_data = {
	.irq_gpio = SYNAPTICS_DSX_I2C_IRQ_GPIO,			  // "synaptics,irq-gpio"
	.irq_on_state = 0,					  // "synaptics,irq-on-state"
	.irq_flags = IRQF_ONESHOT | IRQF_TRIGGER_LOW,		  // "synaptics, irq-gpio"

	.pwr_reg_name = NULL,					  // "synaptics,pwr-reg-name"
	.bus_reg_name = NULL,					  // "synaptics,bus-reg-name"

	.power_gpio = -1,					  // "synaptics,power-gpio"
	.power_on_state = 1,					  // "synaptics,power-on-state"
	.power_delay_ms = 200,					  // "synaptics,power-delay-ms"

	.reset_gpio = SYNAPTICS_DSX_I2C_RES_GPIO,		  // "synaptics,reset-gpio"
	.reset_on_state = 1,					  // "synaptics,reset-on-state"
	.reset_active_ms = 20,					  // "synaptics,reset-active-ms"
	.reset_delay_ms = 200,					  // "synaptics,reset-delay-ms"
	/* Misc. touch configurations */
	.max_y_for_2d = SYNAPTICS_MAX_Y,
	.swap_axes = false,
	.x_flip = false,
	.y_flip = true,
	.ub_i2c_addr = -1,
	...
	/* Virtual button maps */
	.cap_button_map = &_capacitive_button_map,
	.vir_button_map = &_virtual_button_map
};

struct i2c_client* synaptics_dsx_client;
static struct i2c_board_info apl_i2c_devices[] = {
	{
		I2C_BOARD_INFO("synaptics_dsx_i2c", SYNAPTICS_DSX_I2C_TCH_ADDR),
		.irq = SYNAPTICS_DSX_I2C_IRQ_GPIO,
		.platform_data = &_synaptics_platform_data // aka. board_data
	}
};
...
int synaptics_rmi4_bus_init(void)
{
	...
	// Explicitly declare that a device exists out on the bus, so that
	// when a driver comes up
	adapter = i2c_get_adapter(2); // MAGIC
	synaptics_dsx_client = i2c_new_device(adapter, apl_i2c_devices);
	if (synaptics_dsx_client == NULL) {
		printk(KERN_ERR "Error : synaptics_dsx_client is NULL\n");
	}
	i2c_put_adapter(adapter);

	retval = i2c_add_driver(&synaptics_rmi4_i2c_driver);
...
{
	kfree(wr_buf);
	i2c_unregister_device(synaptics_dsx_client);
