...
int synaptics_fw_updater(const unsigned char *fw_data)
{
	...
	pr_notice("%s : Starting Synaptics Firmware Updater\n", __func__);
	if (!fwu)
		return -ENODEV;
	...
	printk(KERN_ERR "%s : %s : start reflash", __FILE__, __FUNCTION__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			 "%s: Allocated %d bytes for image data\n",
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: Firmware Updater Init\n",
			__func__);

	// Explicitly re-add completion CV, since a FWU at start clears the CV,
	// But does not re-declare it. Without the completion re-initialized,
	// removing the fw_update module causes the core_module to panic when
	// trying to cleanup its expansion work queue.
	reinit_completion(&fwu_remove_complete);
...
static void __exit rmi4_fw_update_module_exit(void)
{
	...
	printk(KERN_ERR "%s : %s : module exit\n", __FILE__, __FUNCTION__);
