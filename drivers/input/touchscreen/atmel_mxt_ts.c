			       ...
			       size_t config_mem_size)
{
	...
	while (data_pos < cfg->size) {
		...
		ret = sscanf(cfg->data + data_pos, "%4x %4x %4x%n",
		...
		if (!object) {
			/* Skip object */
			for (i = 0; i < size; i++) {
				ret = sscanf(cfg->data + data_pos, "%2hhx%n",
					     &val, &offset);
				if (ret != 1) {
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg)
{
	...
	/* Read CRCs */
	ret = sscanf(cfg->data + data_pos, "%6x%n", &info_crc, &offset);
	if (ret != 1) {
	...
	ret = sscanf(cfg->data + data_pos, "%6x%n", &config_crc, &offset);
	if (ret != 1) {
