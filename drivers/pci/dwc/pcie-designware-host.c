	...
	for (i = 0; i < num_ctrls; i++) {
		...
		do {
			...
			while ((pos = find_next_bit((unsigned long *) &val, 32,
						    pos)) != 32) {
				irq = irq_find_mapping(pp->irq_domain, i * 32 +
						       pos);
...
static void dw_chained_msi_isr(struct irq_desc *desc)
{
	...
	if (pp->ops->msi_handler)
		pp->ops->msi_handler(pp);
			   ...
			   int where, int size, u32 val)
{
	...
	if (bus->number == pp->root_bus_nr)
		...
		ret = dw_pcie_wr_other_conf(pp, bus, devfn, where, size, val);
	raw_spin_unlock_irqrestore(&pp->lock, flags);
...
void dw_pcie_setup_rc(struct pcie_port *pp)
{
	...
	for (ctrl = 0; ctrl < num_ctrls; ctrl++) {
		dw_pcie_wr_own_conf(pp, PCIE_MSI_INTR0_MASK + (ctrl * 12), 4, ~0);
		dw_pcie_wr_own_conf(pp, PCIE_MSI_INTR0_ENABLE + (ctrl * 12), 4, ~0);
