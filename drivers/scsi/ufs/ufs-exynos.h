...
static inline void phy_pma_writel(struct exynos_ufs *ufs, u32 val, u32 reg)
{
	...
	writel(val, ufs->ufs_phy.reg_pma + reg);
	hci_writel(ufs, reg1 | MPHY_APBCLK_STOP, HCI_CLKSTOP_CTRL);
}
...
static inline u32 phy_pma_readl(struct exynos_ufs *ufs, u32 reg)
{
	...
	hci_writel(ufs, reg1 & ~MPHY_APBCLK_STOP, HCI_CLKSTOP_CTRL);
	reg = readl(ufs->ufs_phy.reg_pma + reg);
	hci_writel(ufs, reg1 | MPHY_APBCLK_STOP, HCI_CLKSTOP_CTRL);
...
static inline void exynos_ufs_enable_ov_tm(struct ufs_hba *hba,
						unsigned int offset)
{
	ufshcd_dme_set(hba, UIC_ARG_MIB(offset), TRUE);
}

static inline void exynos_ufs_disable_ov_tm(struct ufs_hba *hba,
						unsigned int offset)
{
	ufshcd_dme_set(hba, UIC_ARG_MIB(offset), FALSE);
}
...
struct exynos_ufs_uic_attr trav_uic_attr = {
	.tx_trailingclks		= 0x10,
	.tx_dif_p_nsec			= 3000000,	/* unit: ns */
	.tx_dif_n_nsec			= 1000000,	/* unit: ns */
	.tx_high_z_cnt_nsec		= 20000,	/* unit: ns */
