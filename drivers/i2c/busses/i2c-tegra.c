	...
	const char *buf, size_t count)
{
	...
	struct tegra_i2c_bus *i2c_bus = &i2c_dev->busses[0];

	clkval = simple_strtoul(buf, NULL, 10);
	i2c_bus->bus_clk_rate = clkval;
	return count;
}
	...
	char *buf)
{
	...
	return sprintf(buf, "%lu", clk_get_rate(i2c_dev->clk)/8);
}

static DEVICE_ATTR(clock, S_IRUSR | S_IWUSR, show_clock, set_clock);
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_clock);
	if (ret)
		goto err_i2c_sysfs_clock_failed;

	return 0;

err_i2c_sysfs_clock_failed:
err_del_bus:
...
static int tegra_i2c_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_clock);
	kfree(i2c_dev);
	return 0;
}
