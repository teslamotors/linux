...
static struct nand_ecclayout tegra_nand_oob_128 = {
	...
	.eccpos = {
		...
		/* ECC POS is only of size 64 bytes  so commenting the remaining bytes here.
		 * As driver uses the Hardware ECC so it there is no issue with it
		 */
 ...
 * If same address is found then it returns its index, if same address in not present then it looks for
 * unused buffer and returns its index and both if these conditions are not met then index of buffer which has maximum
 * frequency(i.e which was not used for longest time) is returned.
 */
static inline int lru_implementation( long long address,long long *buffer_addresses, int *buffer_frequency ) {
	int i=0;
	...
	return (same_address_index==-1) ? ( (unused_address_index==-1) ? maxfreq_address_index : unused_address_index) : same_address_index ;
}
...
void print_info( struct tegra_nand_info *in ) {
		...
		printk("\n");
		for ( ; i < BUFFER_NUM; i++ ) {
		  printk("buffer_frequency[%d]:%d old_addresses[%d]:0x%llx \n", i, in->buffer_frequency[i], i, in->old_addresses[i] );
		}
		printk("\n");
}
#endif
...
#ifdef CONFIG_MTD_NAND_TEGRA_PAGE_CACHE
static int
prep_and_transfer_dma_cache(struct tegra_nand_info *info, int rx, int do_ecc, uint32_t page,
		  uint32_t column, dma_addr_t data_dma,
		  uint32_t data_len, dma_addr_t oob_dma, uint32_t oob_len, uint8_t *user_addr)
{
	uint32_t i = 0;
	int err;
	/*PAGE_SIZE_2048 = 3 in the Tegra2 TRM */
	...
	/* Start the cache mode */
	{
		...
		if (rx) {
			if (do_ecc)
				info->config_reg |= CONFIG_HW_ERR_CORRECTION;
			/* Just read from the flash to the buffer don't transfer to the controller buffer */
			info->dmactrl_reg |= DMA_CTRL_REUSE_BUFFER;
			writel(NAND_CMD_READ0, CMD_REG1);
			writel(NAND_CMD_READSTART, CMD_REG2);
		}
		...
		if (data_len) {
			...
			info->command_reg |= COMMAND_A_VALID;
			info->dmactrl_reg |= DMA_CTRL_DMA_EN_A;
			/* Configure the DMA for whole mtd->writesize at one go */
		} else {
			return -EINVAL;
		}
		if (oob_len)
	 ...
	 * Issue the dma-read for next page and start the memcpy for the previous
	 * page simultaneously */
	for (i = 0; i < (data_len - info->mtd.writesize)/info->mtd.writesize; i++)
	{
		/* reuse the data_buf as request can be for more than LOGICAL_PAGES_COMBINE
		 * Here j will be use for indexing in the local dma buffer
		 * and i for user_addr buffer */
		...
		if (i)
		{
			if (!j)
			{
				/* We needs to copy the j-1 location which is LOGICAL_PAGES_COMBINE -1 */
				memcpy(user_addr + (i-1) * info->mtd.writesize, info->dma_buf + (LOGICAL_PAGES_COMBINE -1)*info->mtd.writesize, info->mtd.writesize);
			}
			else
				memcpy(user_addr + (i-1) * info->mtd.writesize, info->dma_buf + (j-1) * info->mtd.writesize, info->mtd.writesize);
		}
		if (unlikely(tegra_nand_wait_cmd_done(info))) {
			/* TODO: abort command if needed? */
			pr_err("%s: Timeout while waiting for command\n", __func__);
		...
		dmac_unmap_area(info->dma_buf + j * info->mtd.writesize, info->mtd.writesize, DMA_FROM_DEVICE);
		outer_inv_range(data_dma + j * info->mtd.writesize, data_dma + info->mtd.writesize*(j+1));
	}
	/* Last Page read */
	{
		uint32_t j = i & (LOGICAL_PAGES_COMBINE -1 );
		...
		dmac_map_area(info->dma_buf + j*info->mtd.writesize, info->mtd.writesize, DMA_FROM_DEVICE);
		outer_inv_range(data_dma + j*info->mtd.writesize, data_dma + info->mtd.writesize*(j+1) );
		memcpy(user_addr + (i)*info->mtd.writesize, info->dma_buf + (j)*info->mtd.writesize, info->mtd.writesize);
	}
out_err:
	return err;
}
...
static ssize_t show_flash_size(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct tegra_nand_info *info = dev_get_drvdata(dev);
...
static ssize_t show_num_bad_blocks(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct tegra_nand_info *info = dev_get_drvdata(dev);
	...
	char *buf)
{
	struct tegra_nand_info *info = dev_get_drvdata(dev);
	struct mtd_info *mtd = &info->mtd;
	int num_blocks = mtd->size >> info->chip.block_shift, i, ret = 0, size = 0;
...
do_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if (unlikely(ops->oobbuf && ops->ooblen > oobsz)) {
		...
		page_count = (uint32_t) ((len + mtd->writesize - 1) / mtd->writesize);
	}
	...
	while (page_count--) {
		...
		if (((a_len < mtd->writesize ) && len ) || unaligned ) {
...
#ifdef CONFIG_MTD_NAND_TEGRA_BUFFER_POOL
			address_index_in_buffer = lru_implementation( from, info->old_addresses, info->buffer_frequency );
			if ( ( info->old_addresses[address_index_in_buffer] == from ) && (info->buffer_frequency[address_index_in_buffer] != 0 ) ) {
				datbuf = info->partial_unaligned_rw_buffer + (( mtd->writesize ) * address_index_in_buffer );
				dma = 0;
			}
			else{
				datbuf = info->partial_unaligned_rw_buffer + ((mtd->writesize) * address_index_in_buffer);
			...
			/* buffer_frequency denote the last time it was accessed */
			info->old_addresses[address_index_in_buffer] = from;
		...
		/* Take care when read is of less than page size */
		if (temp_len) {
			memcpy(temp_buf, datbuf + unaligned, temp_len - unaligned);
			a_len = temp_len;
			datbuf = temp_buf;
		}
		if (datbuf) {
...
#ifdef CONFIG_MTD_NAND_TEGRA_PAGE_CACHE
		if(page_count >= (mtd->writebufsize - mtd->writesize) / mtd->writesize )
		{
			dma_addr_t datbuf_dma_addr = info->dma_addr;
			int i =0;
			a_len = page_count * mtd->writesize;
			if (len < a_len)
				a_len -= mtd->writesize;
			err = prep_and_transfer_dma_cache(info, 1, do_ecc, page , column, datbuf_dma_addr,
					a_len, info->oob_dma_addr, 0, datbuf);

			if (err != 0)
				goto out_err;
			for(i = 0 ; i < a_len; i += mtd->writesize)
				correct_ecc_errors_on_blank_page(info, datbuf + i, NULL, mtd->writesize, 0);
			update_ecc_counts(info, oobbuf != NULL);
			len -= a_len;
			datbuf += a_len;
			ops->retlen += a_len;
...
#ifdef CONFIG_MTD_NAND_TEGRA_BUFFER_POOL
        address_index_in_buffer = lru_implementation( to, info->old_addresses, info->buffer_frequency );
#if 0
	print_info(info);
#endif

		/* Take care when write is of less than page size. Otherwise there will be
		 * kernel panic due to dma timeout */
		if (((a_len < mtd->writesize ) && len ) || info->old_addresses[address_index_in_buffer] == to ) {
#else
...
static void
set_chip_timing(struct tegra_nand_info *info, uint32_t vendor_id, uint32_t dev_id, uint32_t fourth_id_field)
{
...
tegra_nand_probe(struct platform_device *pdev)
{
	...
	info->dma_addr = tegra_nand_dma_map(info->dev, info->dma_buf, mtd->writebufsize, DMA_TO_DEVICE);
#endif
...
#ifdef CONFIG_MTD_NAND_TEGRA_BUFFER_POOL
	info->partial_unaligned_rw_buffer = kmalloc((mtd->writesize) * BUFFER_NUM, GFP_KERNEL);
#else
...
#ifdef CONFIG_MTD_NAND_TEGRA_BUFFER_POOL
	/* kzalloc is done so that it initializes the buffers with unused flag(0) */
	info->buffer_frequency = kzalloc(BUFFER_NUM, GFP_KERNEL);
	if(!info->buffer_frequency){
		err = -ENOMEM;
		goto out_free_bbbmap;
	}

	info->old_addresses = kmalloc(sizeof(long long) * BUFFER_NUM, GFP_KERNEL);
