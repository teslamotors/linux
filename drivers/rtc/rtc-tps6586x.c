...
static int tps6586x_set_xtal_load_cap(struct device *tps_dev, int xtal_cap)
{
	int ret;
	uint8_t reg_val;

	ret = tps6586x_read(tps_dev, RTC_CTRL, &reg_val);

	if (ret < 0)
		return ret;

	if (reg_val & RTC_ENABLE)
		ret = tps6586x_update(tps_dev, RTC_CTRL, xtal_cap, RTC_CL_SEL_MASK);
...
static int __devinit tps6586x_rtc_probe(struct platform_device *pdev)
{
	...
	err = tps6586x_set_xtal_load_cap(tps_dev, pdata->xtal_load_capacitance);
	if (err < 0) {
		dev_err(&pdev->dev, "unable to set capacitance load for 32Khz osc src\n");
		goto fail;
	}
