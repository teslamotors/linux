...
#ifdef CONFIG_DHD_USE_STATIC_BUF_TESLA
...
#endif /* CONFIG_DHD_USE_STATIC_BUF_TESLA */
...
#if defined(CONFIG_DHD_USE_STATIC_BUF_TESLA) && defined(DHD_USE_STATIC_CTRLBUF)
...
#endif /* CONFIG_DHD_USE_STATIC_BUF_TESLA && DHD_USE_STATIC_CTRLBUF */
...
osl_dma_map(osl_t *osh, void *va, uint size, int direction, void *p, hnddma_seg_map_t *dmah)
{
	...
	ASSERT((osh && (osh->magic == OS_HANDLE_MAGIC)));
	dir = (direction == DMA_TX)? PCI_DMA_TODEVICE: PCI_DMA_BIDIRECTIONAL;
	...
	hnddma_seg_map_t *dmah, void *ptr_cma_info)
{
	...
	if (sec_mem_elem && sec_mem_elem->va == vaorig) {
		...
		dma_handle = dma_map_page(osh->cma->dev, pa_cma_page, loffset, size,
			(direction == DMA_TX ? DMA_TO_DEVICE:DMA_BIDIRECTIONAL));
	...
	if (direction == DMA_RX) {
		...
		if (p == NULL) {
			...
			*/
			...
			dma_unmap_page(osh->cma->dev, pa_cma, size, DMA_BIDIRECTIONAL);
...
osl_sec_dma_unmap_all(osl_t *osh, void *ptr_cma_info)
{
	...
	while (sec_mem_elem != NULL) {
		...
		dma_unmap_page(osh->cma->dev, sec_mem_elem->pa_cma, sec_mem_elem->size,
			sec_mem_elem->direction == DMA_TX ? DMA_TO_DEVICE : DMA_BIDIRECTIONAL);
		osl_sec_dma_free_mem_elem(osh, sec_mem_elem);
