 ...
 *
 * $Id: dhd_pcie.c 692511 2018-07-19 08:49:15Z $
 */
...
dhdpcie_bus_release(dhd_bus_t *bus)
{
	...
	if (bus) {
		...
		if (bus->dhd) {
			DHD_ERROR(("%s:0515: calling dhd_detach\n", __FUNCTION__));
			dhd_detach(bus->dhd);
			...
			DHD_ERROR(("%s:0515: calling dhdpcie_bus_release_dongle\n", __FUNCTION__));
...
dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	while (((addr == 0) || (addr == bus->nvram_csm)) && !dhd_timeout_expired(&tmo)) {
		...
		addr = LTOH32(dhdpcie_bus_rtcm32(bus, shaddr));
#ifdef PCIE_FULL_DONGLE
		lock_cnt = dhd_os_sdlock_cnt(bus->dhd);
		if (lock_cnt == 1) {
			dhd_os_sdunlock(bus->dhd);
		}
		if (lock_cnt <= 1) {
