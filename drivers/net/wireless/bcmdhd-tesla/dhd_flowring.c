...
int dhd_update_flow_prio_map(dhd_pub_t *dhdp, uint8 map)
{
	...
	if (BCME_OK != dhd_flow_prio_map(dhdp, &map, 1))
		return BCME_ERROR;
...
int dhd_flow_prio_map(dhd_pub_t *dhd, uint8 *map, size_t count)
{
	uint8 iovbuf[24];
	if (count == 0) {
	...
	if (bcm_mkiovar("bus:fl_prio_map", (char *)map, count, (char*)iovbuf,
			sizeof(iovbuf)) == 0) {
		return BCME_ERROR;
	}
