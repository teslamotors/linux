...
static char nv_mac[ETH_ALEN] = {0};
static char nv_bl_macid[BL_MAC_LEN] = {0};
module_param_string(nv_bl_macid, nv_bl_macid, sizeof(nv_bl_macid),
		    0644);
MODULE_PARM_DESC(nv_bl_macid, "macid passed from the bootloader");

static void parse_mac_addr(char *macstr)
{
	int i, j;
...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
	...
	for (i = 0; (i < BL_MAC_LEN) && (nv_bl_macid[i] != ':'); i++);
	if (!strncmp(dev->net->name, nv_bl_macid, i)) {
		parse_mac_addr(nv_bl_macid + i + 1);
		if (is_valid_ether_addr(nv_mac)) {
			memcpy(dev->net->dev_addr, nv_mac, ETH_ALEN);
            netif_dbg(dev, ifup, dev->net, "MAC address set from bootmedium");
			return;
		}
	}
...
static const struct usb_device_id products[] = {
	...
	{
		/* SMSC89530 Automotive version of 9500 */
