...
#define FW_LOAD_XMEM_MAX_TRANSACTION_SUPPORT umin(FW_LOAD_MAX_NB_TRANS, FW_LOAD_MAX_TRANS_SUPPORTED)
	...
	unsigned int n)
{
	...
	begin_count = umin(ia_css_fw_copy_begin_available(), n);
	for ( i= 0; i < begin_count; i++ ) {
		ret = ia_css_fw_copy_begin(
			mmid,
			ssid,
...
{
	return (FW_LOAD_XMEM_MAX_TRANSACTION_SUPPORT - ia_css_fw_copy_end_available());
}
