...
int ia_css_isys_device_open(
	...
	host_virtual_address_t host_firmware_address;
	vied_virtual_address_t vied_firmware_address;
	...
	host_firmware_address = *(unsigned long long *)config->driver_sys.firmware_address;

	/* map firmware on ISYS */
	vied_firmware_address = shared_memory_map(ssid, mmid, host_firmware_address);

	/* configure master port registers */
	ia_css_isys_set_master_port_regs(config->driver_sys.ssid);

	verifret(config->mipi.nof_blocks > 0, EINVAL);
	verifret(config->mipi.nof_blocks <= NOF_SRAM_BLOCKS_MAX, EINVAL);
	for (block_id = 0; block_id < config->mipi.nof_blocks; block_id++) {
		verifret(config->mipi.block_size[block_id] > 0, EINVAL);
	}
	verifret(config->pixel.nof_blocks > 0, EINVAL);
	verifret(config->pixel.nof_blocks <= NOF_SRAM_BLOCKS_MAX, EINVAL);
	for (block_id = 0; block_id < config->pixel.nof_blocks; block_id++) {
		verifret(config->pixel.block_size[block_id] > 0, EINVAL);
	}
	...
	*context = (HANDLE)ctx;
	...
	sys.vied_firmware_address = vied_firmware_address;
	...
	/* load the firmware, no start */
	ia_css_fw_load_init();
	if (ia_css_cell_program_load(ssid, mmid, host_firmware_address, vied_firmware_address) != 0)
		return ENOEXEC;
