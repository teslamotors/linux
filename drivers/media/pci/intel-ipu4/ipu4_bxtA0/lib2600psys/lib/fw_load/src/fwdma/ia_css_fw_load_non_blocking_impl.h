	...
	unsigned int size)
{
	...
	if (no_of_request >= IA_CSS_FW_DMA_MAX_TRANSACTION_SUPPORT) {
		return 0; /* cannot submit request if not enough resources */
	}
	fw_dma_transact(
		...
		size);
	no_of_request++;
	return 1;/* ia_css_fw_copy_begin only supports single request at time */
}
	...
	unsigned int n)
{
	...
	for (i = 0; i < submit_req; i++) {
		OP___dump(__LINE__, dev_acc_fw_load[i].src);
		OP___dump(__LINE__, dev_acc_fw_load[i].dst);
		OP___dump(__LINE__, dev_acc_fw_load[i].size);
		ia_css_fw_copy_begin(
			mmid,
			ssid,
