...
/** all terminals */
struct sh_css_isp_ls_isp_config {
	struct ia_css_terminal_descriptor_s config[MAX_NUM_LS];
};


/* DMA based terminals */
struct ia_css_common_dma_config {
	unsigned ddr_elems_per_word[MAX_NUM_IO];
	unsigned dma_channel[MAX_NUM_IO][MAX_IO_DMA_CHANNELS];
	unsigned here_crop_x[MAX_NUM_IO]; /* horizontal cropping in terms of elements on ISP side */
	unsigned here_crop_y[MAX_NUM_IO]; /* vertical cropping in terms of elements on ISP side */
};

struct ia_css_common_plane_config {
