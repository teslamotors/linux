	...
	const ia_css_program_group_param_t		*param)
{
	...
	if (ia_css_is_terminal_manifest_parameter_terminal(manifest)) {
		...
		size = ia_css_program_terminal_get_descriptor_size(fragment_count, prog_term_man->fragment_param_manifest_section_desc_count,
			prog_term_man->kernel_fragment_sequencer_info_manifest_info_count, 0 /* Pool of commands is not neeed for now in PSYS */);
	...
	ia_css_kernel_bitmap_t				enable_bitmap)
{
	...
	if (ia_css_is_terminal_manifest_data_terminal(manifest) == true) {
		...
		ia_css_terminal_manifest_get_type(manifest) == IA_CSS_TERMINAL_TYPE_PARAM_CACHED_IN) {
		...
		IA_CSS_TRACE_0(PSYSAPI_DYNAMIC, INFO, "ia_css_is_terminal_manifest_program_terminal(manifest) == true \n");
		...
		prog_terminal->fragment_param_section_desc_offset =
			...
			(fragment_count * prog_terminal_man->kernel_fragment_sequencer_info_manifest_info_count * sizeof(ia_css_kernel_fragment_sequencer_info_desc_t)) +
			(0 /*nof_command_objs */ * sizeof(ia_css_kernel_fragment_sequencer_command_desc_t));

	} else if (ia_css_is_terminal_manifest_spatial_parameter_terminal(manifest) == true &&
		ia_css_terminal_manifest_get_type(manifest) == IA_CSS_TERMINAL_TYPE_PARAM_SPATIAL_IN) {
...
uint16_t ia_css_terminal_get_terminal_manifest_index(
	const ia_css_terminal_t					*terminal)
{
	uint16_t	terminal_manifest_index = (uint16_t)~0;
