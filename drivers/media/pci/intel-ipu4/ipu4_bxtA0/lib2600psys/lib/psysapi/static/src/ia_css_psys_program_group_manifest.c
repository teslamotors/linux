...
size_t ia_css_sizeof_program_group_manifest(
	...
	const uint16_t							param_section_count,
	const uint16_t							param_out_section_count,
	...
	const uint16_t							kernel_fragment_seq_count)
{
	...
	NOT_USED(param_out_section_count);
	...
	for (i = 0; i < (int)terminal_count; i++) {
		...
		case IA_CSS_TERMINAL_TYPE_PARAM_CACHED_OUT:
			size += ia_css_param_terminal_manifest_get_size(param_section_count);
			break;
	...
	const uint16_t			kernel_fragment_seq_count)
{
	...
	for (i = 0; i < (int)terminal_count; i++) {
		...
		case IA_CSS_TERMINAL_TYPE_PARAM_CACHED_IN:
		case IA_CSS_TERMINAL_TYPE_PARAM_CACHED_OUT:
			result = ia_css_param_terminal_manifest_init((ia_css_param_terminal_manifest_t *)term_manifest, param_section_count);
			if(0 == result) {
				terminal_size = ia_css_param_terminal_manifest_get_size(param_section_count);
			} else {
		...
		case IA_CSS_TERMINAL_TYPE_PARAM_SPATIAL_OUT:
			...
			if(0 == result) {
				...
				IA_CSS_TRACE_0(PSYSAPI_STATIC, ERROR, "ia_css_spatial_param_terminal_manifest_init failed\n");
			}
			break;
		case IA_CSS_TERMINAL_TYPE_PROGRAM:
			...
			if(0 == result) {
				...
				IA_CSS_TRACE_0(PSYSAPI_STATIC, ERROR, "ia_css_program_terminal_manifest_init failed\n");
			}
			break;
		case IA_CSS_TERMINAL_TYPE_DATA_IN:
		case IA_CSS_TERMINAL_TYPE_DATA_OUT:
