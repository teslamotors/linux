			...
			vied_virtual_address_t vied_pkg_dir_address)
{
	...
	/* check entry type */
	if (ia_css_pkg_dir_entry_get_type(server_entry) != IA_CSS_PKG_DIR_PSYS_SERVER_PG)
		return 1;
	...
	struct ia_css_syscom_config *config)
{
	...
	ia_css_cell_set_master_info_bits(SSID, SPC0, IPU_DEVICE_SP2600_CONTROL_XMEM, IA_CSS_INFO_BITS_M0_DDR);
