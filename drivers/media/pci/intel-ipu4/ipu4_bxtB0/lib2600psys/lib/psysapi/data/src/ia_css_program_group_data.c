...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_print failed (%i)\n", retval);
	}
	return retval;
}
...
EXIT:
	if (NULL == frame) {
		IA_CSS_TRACE_0(PSYSAPI_DATA, WARNING, "ia_css_frame_get_buffer_host_virtual_address invalid argument\n");
	}
	return NULL;
}
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_set_buffer failed (%i)\n", retval);
	}
	return retval;
}
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_set_data_bytes failed (%i)\n", retval);
	}
	return retval;
}
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_set_buffer_state failed (%i)\n", retval);
	}
	return retval;
}
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_set_pointer_state failed (%i)\n", retval);
	}
	return retval;
}
	...
	void									*fid)
{
	...
	IA_CSS_TRACE_0(PSYSAPI_DATA, INFO, "ia_css_frame_descriptor_print(): enter:\n");
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_frame_descriptor_print failed (%i)\n", retval);
	}
	return retval;
}
	...
	void									*fid)
{
	...
	IA_CSS_TRACE_0(PSYSAPI_DATA, INFO, "ia_css_fragment_descriptor_print(): enter:\n");
	...
	IA_CSS_TRACE_1(PSYSAPI_DATA, INFO, "index[%d] = {\n", IA_CSS_N_DATA_DIMENSION);
	for (i = 0; i < (int)IA_CSS_N_DATA_DIMENSION - 1; i++) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, INFO, "\t%4d,\n", fragment_descriptor->index[i]);
	}
	IA_CSS_TRACE_1(PSYSAPI_DATA, INFO, "\t%4d }\n", fragment_descriptor->index[i]);

	IA_CSS_TRACE_1(PSYSAPI_DATA, INFO, "offset[%d] = {\n", IA_CSS_N_DATA_DIMENSION);
	for (i = 0; i < (int)IA_CSS_N_DATA_DIMENSION - 1; i++) {
...
EXIT:
	if (retval != 0) {
		IA_CSS_TRACE_1(PSYSAPI_DATA, ERROR, "ia_css_fragment_descriptor_print failed (%i)\n", retval);
	}
	return retval;
}
	...
	const ia_css_frame_format_type_t		frame_format_type)
{
	...
	if ((frame_format_type < IA_CSS_N_FRAME_FORMAT_TYPES) && (frame_format_type < IA_CSS_FRAME_FORMAT_BITMAP_BITS)) {
		...
		IA_CSS_TRACE_0(PSYSAPI_DATA, WARNING, "ia_css_frame_format_bit_mask invalid argument\n");
	}
