...
/*! Get the (pointer to) private data blob in the manifest
 ...
 @return private data blob, NULL on error
 */
...
/*! Get the (pointer to) indexed spatial param terminal manifest in the program group manifest object

 @param	manifest[in]			program group manifest object
 @param	program_index[in]		index of the terminal manifest object

 @return spatial param terminal manifest, NULL on error
 */
extern ia_css_spatial_param_terminal_manifest_t *ia_css_program_group_manifest_get_spatial_param_terminal_manifest(
	const ia_css_program_group_manifest_t	*manifest,
	const unsigned int						terminal_index);

/*! Get the (pointer to) indexed sliced param terminal manifest in the program group manifest object

 @param	manifest[in]			program group manifest object
 @param	program_index[in]		index of the terminal manifest object

 @return sliced param terminal manifest, NULL on error
 */
ia_css_sliced_param_terminal_manifest_t *ia_css_program_group_manifest_get_sliced_param_terminal_manifest(
	const ia_css_program_group_manifest_t	*manifest,
	const unsigned int			terminal_index);

/*! Get the (pointer to) indexed program terminal manifest in the program group manifest object

  @parammanifest[in]program group manifest object
  @paramprogram_index[in]index of the terminal manifest object

  @return program terminal manifest, NULL on error
*/
extern ia_css_program_terminal_manifest_t *ia_css_program_group_manifest_get_program_terminal_manifest(
	const ia_css_program_group_manifest_t*manifest,
	const unsigned intterminal_index);
...
/*!	initialize program group manifest
 ...
 @param	spatial_param_section_count[in]		Array[spatial_terminal_count] with sections per cached out terminal
 @param sliced_in_param_section_count[in]	Array[sliced_in_terminal_count] with sections per sliced in terminal
 @param sliced_out_param_section_count[in]	Array[sliced_out_terminal_count] with sections per sliced out terminal
 @param	fragment_param_section_count[in]	Number of fragment parameter sections of the program init terminal,
 @param	kernel_fragment_seq_count[in]		Number of kernel fragment seqence info.
 @return none;
 */
