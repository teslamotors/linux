...
STORAGE_CLASS_INLINE void __dummy_check_alignment(void) {
	...
	COMPILATION_ERROR_IF(0 != sizeof(ia_css_param_terminal_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_PARAM_SEC_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_param_section_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_param_section_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_SPATIAL_PARAM_TERM_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_spatial_param_terminal_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_spatial_param_terminal_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAME_GRID_PARAM_SEC_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_frame_grid_param_section_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_frame_grid_param_section_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAG_GRID_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_fragment_grid_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_fragment_grid_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_SLICED_PARAM_TERM_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_sliced_param_terminal_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_sliced_param_terminal_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAGMENT_SLICE_DESC_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_fragment_slice_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_fragment_slice_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_SLICE_PARAM_SECTION_DESC_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_slice_param_section_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_slice_param_section_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_PROG_TERM_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_program_terminal_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_program_terminal_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAG_SEQ_INFO_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_kernel_fragment_sequencer_info_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_kernel_fragment_sequencer_info_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAG_SEQ_COMMANDS_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_kernel_fragment_sequencer_command_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_kernel_fragment_sequencer_command_desc_t)%sizeof(uint64_t));

	COMPILATION_ERROR_IF(
		SIZE_OF_FRAG_PARAM_SEC_STRUCT_BITS != (CHAR_BIT * sizeof(ia_css_fragment_param_section_desc_t)));

	COMPILATION_ERROR_IF(0 != sizeof(ia_css_fragment_param_section_desc_t)%sizeof(uint64_t));
...
{
	ia_css_param_section_desc_t *param_section_base, *param_section_desc = NULL;

	verifjmpexit(param_terminal != NULL);

	param_section_base = (ia_css_param_section_desc_t *)(((const char *)param_terminal) + param_terminal->param_section_desc_offset);
...
)
{
	...
	param_section_base = (ia_css_param_section_desc_t *)(((const char *)param_terminal) + param_terminal->param_section_desc_offset);
	param_section_desc = &(param_section_base[(nof_sections * fragment_index) + section_index]);
...
)
{
	...
	param_terminal->base.parent_offset = 0-((int16_t)terminal_offset);
	param_terminal->base.size = terminal_size;
	param_terminal->param_section_desc_offset = sizeof(ia_css_param_terminal_t);
...
)
{
	return sizeof(ia_css_spatial_param_terminal_t) +
		nof_frame_param_sections*sizeof(ia_css_frame_grid_param_section_desc_t) +
		nof_fragments*sizeof(ia_css_fragment_grid_desc_t);
}

ia_css_fragment_grid_desc_t *ia_css_spatial_param_terminal_get_fragment_grid_desc(
	const ia_css_spatial_param_terminal_t *spatial_param_terminal,
	const unsigned int fragment_index
)
{
	ia_css_fragment_grid_desc_t *fragment_grid_desc_base, *fragment_grid_desc = NULL;

	verifjmpexit(spatial_param_terminal != NULL);

	fragment_grid_desc_base = (ia_css_fragment_grid_desc_t *)(((const char *)spatial_param_terminal) + spatial_param_terminal->fragment_grid_desc_offset);
...
{
	ia_css_frame_grid_param_section_desc_t *frame_grid_param_section_base, *frame_grid_param_section_desc = NULL;

	verifjmpexit(spatial_param_terminal != NULL);

	frame_grid_param_section_base = (ia_css_frame_grid_param_section_desc_t *)(((const char *)spatial_param_terminal) +
		spatial_param_terminal->frame_grid_param_section_desc_offset);
	frame_grid_param_section_desc = &(frame_grid_param_section_base[section_index]);
...
)
{
	...
	spatial_param_terminal->base.parent_offset = 0-((int16_t)terminal_offset);
...
)
{
	...
	for (fragment_index = 0; fragment_index < nof_fragments; fragment_index++) {
...
{
	ia_css_fragment_slice_desc_t *fragment_slice_desc_base, *fragment_slice_desc = NULL;

	verifjmpexit(sliced_param_terminal != NULL);

	fragment_slice_desc_base = (ia_css_fragment_slice_desc_t *)(((const char *)sliced_param_terminal) + sliced_param_terminal->fragment_slice_desc_offset);
...
)
{
	...
	ia_css_slice_param_section_desc_t *slice_param_section_desc_base, *slice_param_section_desc = NULL;
	...
	slice_param_section_desc_base = (ia_css_slice_param_section_desc_t *)(((const char *)sliced_param_terminal) + fragment_slice_desc->slice_section_desc_offset);
	slice_param_section_desc = &(slice_param_section_desc_base[(slice_index * nof_slice_param_sections) + section_index]);
...
)
{
	...
	sliced_param_terminal->base.parent_offset = 0-((int16_t)terminal_offset);
	sliced_param_terminal->base.size = terminal_size;
	sliced_param_terminal->kernel_id = kernel_id;
	sliced_param_terminal->fragment_slice_desc_offset = sizeof(ia_css_sliced_param_terminal_t);	/* set here to use below to find the pointer */
	for (fragment_index = 0; fragment_index < nof_fragments; fragment_index++) {
		ia_css_fragment_slice_desc_t *fragment_slice_desc =
			ia_css_sliced_param_terminal_get_fragment_slice_desc(
				sliced_param_terminal,
...
)
{
	return sizeof(ia_css_program_terminal_t) +
		nof_fragments*nof_fragment_param_sections*sizeof(ia_css_fragment_param_section_desc_t) +
		nof_fragments*nof_kernel_fragment_sequencer_infos*sizeof(ia_css_kernel_fragment_sequencer_info_desc_t) +
		nof_command_objs*sizeof(ia_css_kernel_fragment_sequencer_command_desc_t);
}

ia_css_fragment_param_section_desc_t *ia_css_program_terminal_get_fragment_param_section_desc(
...
{
	ia_css_fragment_param_section_desc_t *fragment_param_section_desc_base, *fragment_param_section_desc = NULL;

	verifjmpexit(program_terminal != NULL);
	verifjmpexit(section_index < nof_fragment_param_sections);

	fragment_param_section_desc_base = (ia_css_fragment_param_section_desc_t *)(((const char *)program_terminal) +
		program_terminal->fragment_param_section_desc_offset);
	fragment_param_section_desc = &(fragment_param_section_desc_base[(fragment_index * nof_fragment_param_sections) + section_index]);
...
)
{
	...
	kernel_fragment_sequencer_info_desc = &(kernel_fragment_sequencer_info_desc_base[(fragment_index * nof_kernel_fragment_sequencer_infos) + info_index]);
...
)
{
	...
	program_terminal->kernel_fragment_sequencer_info_desc_offset = sizeof(ia_css_program_terminal_t);
	program_terminal->fragment_param_section_desc_offset =
		program_terminal->kernel_fragment_sequencer_info_desc_offset +
		(nof_fragments * nof_kernel_fragment_sequencer_infos * sizeof(ia_css_kernel_fragment_sequencer_info_desc_t)) +
		(nof_command_objs * sizeof(ia_css_kernel_fragment_sequencer_command_desc_t));
...
)
{
	...
	*command_desc_offset =
		program_terminal->kernel_fragment_sequencer_info_desc_offset +
		(nof_fragments * nof_kernel_fragment_sequencer_infos * sizeof(ia_css_kernel_fragment_sequencer_info_desc_t)) +
		(commands_slots_used * sizeof(ia_css_kernel_fragment_sequencer_command_desc_t));
...
)
{
	...
	line_count = (uint16_t *)&(kernel_fragment_sequencer_command_desc_base[set_count >> 2].line_count[set_count & 0x00000003]);
EXIT:
	return line_count;
}
