...
#ifndef _IA_CSS_CELL_PROGRAM_GROUP_LOAD_IMPL_H_
#define _IA_CSS_CELL_PROGRAM_GROUP_LOAD_IMPL_H_
	...
	unsigned int vied_addr)
{
	...
	if (next) {
		status = ia_css_cell_program_group_load(ssid, mmid, host_addr + next, vied_addr + next);
		if (status)
			return status;
	}
...
#endif /* _IA_CSS_CELL_PROGRAM_GROUP_LOAD_IMPL_H_ */
