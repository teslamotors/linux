...
#define IMX274_REG_XMSTA                0x30F4
#define IMX274_REG_SHR_L                0x300C  /* L/R mode */
#define IMX274_REG_SHR_R                0x300D  /* L/R mode */
#define IMX274_REG_SYNCDRV              0x3018
...
static struct crl_register_write_rep imx274_pll_1440mbps[] = {
	...
	{0x312B, CRL_REG_LEN_08BIT, 0x03},
	{0x312D, CRL_REG_LEN_08BIT, 0x02},
	{0x3AC4, CRL_REG_LEN_08BIT, 0x00},
};

/* This format settings is not minimum,which has redundent settings */
static struct crl_register_write_rep imx274_fmt_raw10[] = {
	{0x3342, CRL_REG_LEN_08BIT, 0x0A},	/* FMT RAW10 */
	{0x3343, CRL_REG_LEN_08BIT, 0x00},
	{0x3528, CRL_REG_LEN_08BIT, 0x0E},
	{0x3555, CRL_REG_LEN_08BIT, 0x01},
	{0x3556, CRL_REG_LEN_08BIT, 0x01},
	{0x3667, CRL_REG_LEN_08BIT, 0x01},
...
static struct crl_register_write_rep imx274_fmt_raw12[] = {
	{0x3342, CRL_REG_LEN_08BIT, 0xFF},	/* FMT RAW12 */
	{0x3343, CRL_REG_LEN_08BIT, 0x01},
	{0x3528, CRL_REG_LEN_08BIT, 0x0F},
...
static struct crl_register_write_rep imx274_3864_2202_27MHZ_CROPPING[] = {
	...
	{0x3004, CRL_REG_LEN_08BIT, 0x00},	/* MDSEL */
	...
	{0x30F8, CRL_REG_LEN_08BIT, 0x08},	/* VMAX */
	{0x30F9, CRL_REG_LEN_08BIT, 0x13},
...
static struct crl_register_write_rep imx274_1932_1094_27MHZ_30FR_CROPPING[] = {
	...
	{0x3005, CRL_REG_LEN_08BIT, 0x61},
	{0x3006, CRL_REG_LEN_08BIT, 0x00},
	{0x3007, CRL_REG_LEN_08BIT, 0x19},
	{0x300E, CRL_REG_LEN_08BIT, 0x03},	/* SVR */
	{0x300F, CRL_REG_LEN_08BIT, 0x00},
	{0x3037, CRL_REG_LEN_08BIT, 0x00},	/* HTRIM reference HTRIM_EN */
	{0x3038, CRL_REG_LEN_08BIT, 0x00},	/* HTRIM_START */
	{0x3039, CRL_REG_LEN_08BIT, 0x00},
	...
	{0x30F6, CRL_REG_LEN_08BIT, 0x04},	/* HMAX */
	{0x30F7, CRL_REG_LEN_08BIT, 0x01},
	{0x30F8, CRL_REG_LEN_08BIT, 0x06},	/* VMAX */
...
static struct crl_arithmetic_ops imx274_fll_hsb_ops[] = {
	{
		.op = CRL_BITWISE_RSHIFT,
...
static struct crl_arithmetic_ops imx274_ana_msb_ops[] = {
	{
		.op = CRL_BITWISE_RSHIFT,
		.operand.entity_val = 8,
	}
...
static struct crl_dynamic_register_access imx274_ana_gain_global_regs[] = {
	...
	},
	{
		.address = 0x300B,
		.len = CRL_REG_LEN_08BIT,
		.ops_items = ARRAY_SIZE(imx274_ana_msb_ops),
		.ops = imx274_ana_msb_ops,
		.mask = 0x07,
	},
};
...
static struct crl_dynamic_register_access imx274_fll_regs[] = {
	...
	},
	{
		.address = 0x30F9,
		.len = CRL_REG_LEN_08BIT,
		.ops_items = ARRAY_SIZE(imx274_fll_msb_ops),
		.ops = imx274_fll_msb_ops,
		.mask = 0xff,
	},
	{
		.address = 0x30FA,
		.len = CRL_REG_LEN_08BIT,
		.ops_items = ARRAY_SIZE(imx274_fll_hsb_ops),
		.ops = imx274_fll_hsb_ops,
		.mask = 0xf,
	},
};
...
static struct crl_pll_configuration imx274_pll_configurations[] = {
	{
		...
		.pixel_rate_pa = 480000000,	/* MIPICLK*2 *csi_lanes/bpp */
		.csi_lanes = 4,
		.comp_items = 0,
		.ctrl_data = 0,
	...
	},
	{
		...
		.op_sys_clk = 720000000,	/* 1440000000/2 */
		.bitsperpixel = 10,
		.pixel_rate_csi = 576000000,
		.pixel_rate_pa = 576000000,	/* MIPICLK*2 *csi_lanes/bpp */
		.csi_lanes = 4,
		.comp_items = 0,
		.ctrl_data = 0,
		.pll_regs_items = ARRAY_SIZE(imx274_pll_1440mbps),
...
/* pixel area is 3864*2202 effective area is 3840*2160 */
...
static struct crl_csi_data_fmt imx274_crl_csi_data_fmt[] = {
	{
		...
		.pixel_order = CRL_PIXEL_ORDER_GRBG,
		.bits_per_pixel = 10,
		.regs_items = ARRAY_SIZE(imx274_fmt_raw10),
		.regs = imx274_fmt_raw10,
	},
	{
	...
	},
	{
		...
		.pixel_order = CRL_PIXEL_ORDER_GRBG,
		.bits_per_pixel = 12,
		.regs_items = ARRAY_SIZE(imx274_fmt_raw12),
		.regs = imx274_fmt_raw12,
	},
	{
...
static struct crl_v4l2_ctrl imx274_v4l2_ctrls[] = {
	...
	},
	{
		...
		.impact = CRL_IMPACTS_NO_IMPACT,
		.ctrl = 0,
		.regs_items = ARRAY_SIZE(imx274_v_flip_regs),
		.regs = imx274_v_flip_regs,
		.dep_items = 0,
		.dep_ctrls = 0,
	},
	{
	...
	},
	{
		...
		.type = CRL_V4L2_CTRL_TYPE_CUSTOM,
		.data.std_data.min = 2224,
		.data.std_data.max = IMX274_HMAX,
		.data.std_data.step = 1,
		.data.std_data.def = 4200,
		.flags = V4L2_CTRL_FLAG_UPDATE,
		.impact = CRL_IMPACTS_NO_IMPACT,
		.ctrl = 0,
