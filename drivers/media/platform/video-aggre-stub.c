				...
				struct v4l2_mbus_frame_desc *desc)
{
	...
	for (i = 0; i < min_t(unsigned int, va->nstreams, desc->num_entries);
	     i++) {
		struct v4l2_mbus_framefmt *ffmt =
...
{
	int rval;
	const struct v4l2_ctrl_config *ctrl = video_aggre_controls;
...
#else
	rval = media_entity_pads_init(&va->sd.entity, 1, &va->pad);
#endif
