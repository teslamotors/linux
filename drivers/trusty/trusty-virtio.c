...
struct trusty_ctx {
	...
	void			*shared_va;
	size_t			shared_sz;
	struct swork_event	check_vqs;
	struct swork_event	kick_vqs;
...
static void check_all_vqs(struct swork_event *work)
{
	uint i;
...
static void kick_vqs(struct swork_event *work)
{
	uint i;
	struct trusty_vdev *tvdev;
	struct trusty_ctx *tctx = container_of(work, struct trusty_ctx,
...
static int trusty_virtio_probe(struct platform_device *pdev)
{
	...
	INIT_SWORK(&tctx->check_vqs, check_all_vqs);
	INIT_SWORK(&tctx->kick_vqs, kick_vqs);
