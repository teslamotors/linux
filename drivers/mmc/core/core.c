...
void mmc_refresh_work(struct work_struct *work)
{
	...
	if (card->ext_csd.refresh == MMC_REFRESH_HYNIX_F20) {
		mmc_claim_host(card->host);
		printk(KERN_DEBUG "%s: refresh start for Hynix f20\n",
				mmc_hostname(card->host));
		err = mmc_send_manufacturer_cmd(card, MMC_MANUF_CMD60,
				MMC_HYNIX_F20_CMD60_START);
		if (err) {
			mmc_release_host(card->host);
			goto out;
		}
		err = mmc_send_manufacturer_cmd(card, MMC_MANUF_CMD60,
				MMC_HYNIX_F20_CMD60_REFRESH);
		if (err) {
			mmc_release_host(card->host);
			goto out;
		}
		args = 0x55; /* refresh for MLC */
		err = mmc_send_switch_cmd(card, args & 0xFF,
				args >> 16 & 0xFF, args >> 8 & 0xFF,
				args >> 24 & 0xFF);

		printk(KERN_DEBUG "%s: refresh end for Hynix f20\n",
				mmc_hostname(card->host));
		mmc_release_host(card->host);
	} else if (card->ext_csd.refresh == MMC_REFRESH_HYNIX_F26) {
		printk(KERN_DEBUG "%s: refresh for Hynix f26\n",
				mmc_hostname(card->host));
		err = mmc_gen_cmd(card, buf, 0x44, 0x1, 0x0, 0x1);
	}
out:
	if (err)
		printk(KERN_ERR "%s: command response has error: %d\n",
				__func__, err);
	return;
}
...
void mmc_refresh(unsigned long data)
{
	...
	if (card->ext_csd.refresh == MMC_REFRESH_HYNIX_F26) {
	...
	} else if (card->ext_csd.refresh == MMC_REFRESH_HYNIX_F20)
		timeout = timeout1;
...
int mmc_gen_cmd_args(struct mmc_card *card, void *buf, u8 index, u8 arg1,
			u8 arg2, u8 mode)
{
	if (card->type == MMC_TYPE_MMC)
		return mmc_gen_cmd(card, buf, index, arg1, arg2, mode);

	printk(KERN_ERR "mmc_gen_cmd: error: command supported only on MMC "
			"cards\n");
	return -EINVAL;
}
EXPORT_SYMBOL(mmc_gen_cmd_args);
