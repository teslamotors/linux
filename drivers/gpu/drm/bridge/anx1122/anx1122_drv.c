...
static struct gpio edp_reset = { .gpio = 453, .label = "bmp-edp-reset" };
...
#endif
static struct anx1122_reg_val anx1122_reg_values[] = {
...
static int anx1122_attach(struct drm_bridge *bridge)
{
	struct anx1122_bridge *anx = bridge_to_anx1122(bridge);
	int ret = 0;

	ret = anx1122_configure(anx);
	if (ret < 0)
		return -EIO;
...
static void anx1122_detach(struct drm_bridge *bridge)
{
	struct anx1122_bridge *anx = bridge_to_anx1122(bridge);
	i2c_unregister_device(anx->clients.access);
}

static int anx1122_resume(struct drm_bridge *bridge)
{
	struct anx1122_bridge *anx = bridge_to_anx1122(bridge);
	int ret;
	...
	return anx1122_configure(anx);
}

static struct drm_bridge_funcs anx1122_bridge_funcs = {
	.attach = anx1122_attach,
	.detach = anx1122_detach,
	.resume = anx1122_resume,
};
...
#ifdef CONFIG_PM
static int anx1122_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct anx1122_bridge *anx = i2c_get_clientdata(client);
	int ret = 0;

	ret = regulator_disable(anx->reg_vid_en);
	if (ret < 0)
		DRM_ERROR("%s failed to disable regulator, %d\n", __func__,
				ret);

	return ret;
}

static struct i2c_board_info anx1122_i2c_board_info[] = {
	{ I2C_BOARD_INFO("anx1122", ANX1122_I2C_ADDR_ACCESS) },
};

static const struct dev_pm_ops anx1122_pm_ops = {
	.suspend = anx1122_suspend,
};
#endif
