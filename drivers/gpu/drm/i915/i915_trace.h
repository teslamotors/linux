...
DECLARE_EVENT_CLASS(i915_gem_request,
	    ...
	    TP_printk("dev=%u, ring=%u, ctx=%u, seqno=%u, global=%u, priority=%d (%d)",
		      __entry->dev, __entry->ring, __entry->ctx, __entry->seqno,
		      __entry->global, __entry->prio_req, __entry->prio_ctx)
);
...
DEFINE_EVENT(i915_gem_request, i915_gem_request_preempt,
	     TP_PROTO(struct drm_i915_gem_request *req),
	     TP_ARGS(req)
);

DEFINE_EVENT(i915_gem_request, i915_gem_request_forcepreempt,
	     TP_PROTO(struct drm_i915_gem_request *req),
	     TP_ARGS(req)
);
#else
...
static inline void
trace_i915_gem_request_preempt(struct drm_i915_gem_request *req)
{
}

static inline void
trace_i915_gem_request_forcepreempt(struct drm_i915_gem_request *req)
{
}
#endif
#endif
...
TRACE_EVENT(intel_engine_notify,
	    ...
	    TP_printk("dev=%u, ring=%u, global=%u, waiters=%u",
...
DEFINE_EVENT(i915_gem_request, i915_gem_request_unsubmit,
	    TP_PROTO(struct drm_i915_gem_request *req),
	    TP_ARGS(req)
);
...
DECLARE_EVENT_CLASS(i915_engine,
	...
	TP_STRUCT__entry(
			...
			__field(u32, fence_seqno)
			__field(u32, hws_seqno)
			__field(u32, inflight_seqnos)
	),

	TP_fast_assign(
			__entry->ring = engine->id;
			__entry->dev = engine->i915->drm.primary->index;
			__entry->fence_context = engine->timeline->fence_context;
			__entry->fence_seqno = engine->timeline->seqno;
			__entry->hws_seqno = intel_engine_get_seqno(engine);
			__entry->inflight_seqnos = engine->timeline->inflight_seqnos;
	),

	TP_printk("dev=%u, ring=%u, fence_context=%lld, timeline=%d, global=%d, inflight=%d",
		  __entry->dev, __entry->ring,
		  __entry->fence_context, __entry->fence_seqno, __entry->hws_seqno,
		  __entry->inflight_seqnos)
);

DEFINE_EVENT(i915_engine, i915_preempt_postprocess,
	    TP_PROTO(struct intel_engine_cs *engine),
	    TP_ARGS(engine)
);

DEFINE_EVENT(i915_engine, i915_guc_preempt,
	    TP_PROTO(struct intel_engine_cs *engine),
	    TP_ARGS(engine)
);

TRACE_EVENT(i915_preempt_type,
	    TP_PROTO(enum intel_preempt_type type, u32 cmd),
	    TP_ARGS(type, cmd),

	    TP_STRUCT__entry(
			     __field(enum intel_preempt_type, type)
			     __field(u32, cmd)
			     ),
	    ...
	    TP_printk("type=%s, cmd=%x",
		      preempt_type_to_str(__entry->type), __entry->cmd)
);

TRACE_EVENT(i915_guc_send,
		    TP_PROTO(uint32_t action, int ret, uint32_t status),
		    TP_ARGS(action, ret, status),
		    ...
		    TP_printk("action=%u, ret=%d, status=0x%08x",
			      __entry->action, __entry->ret, __entry->status)
);
...
TRACE_EVENT(i915_guc_submit,
	    TP_PROTO(struct drm_i915_gem_request *req, int ret),
	    TP_ARGS(req, ret),
	    ...
	    TP_printk("dev=%u, ring=%u, seqno=%u, ret=%d",
		      __entry->dev, __entry->ring,
		      __entry->seqno, __entry->ret)
);
