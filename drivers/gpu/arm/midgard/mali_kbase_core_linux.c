...
static int assign_irqs(struct platform_device *pdev)
{
	...
	for (i = 0; i < 3; i++) {
		...
		snprintf(kbdev->irqs[irqtag].irq_name,
				 sizeof(kbdev->irqs[irqtag].irq_name),
				 "%s-%s", kbdev->devname, irq_res->name);
	}
...
static int kbase_platform_device_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < 4; i++) {
		iounmap(kbdev->regnoc[i]);
		iounmap(kbdev->regack[i]);
	}
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	if (!of_property_read_u32(pdev->dev.of_node, "pwr-policy",
			&kbdev->default_pwr_policy))
		dev_info(kbdev->dev, "Using pwr-policy %d set from dtb",
					kbdev->default_pwr_policy);
