...
#include <platform/trav/gpu_integration_defs.h>
...
struct kbase_device {
	...
	struct {
		int irq;
		int flags;
		char irq_name[DEVNAME_SIZE];
	} irqs[3];

	/* MAII_SEC_INTEGRATION */
	/* Adding mali clock to use them during devfreq */
	struct clk *clock;
	struct clk *mout_clk_gpu_switch_user;
	struct clk *mout_clk_gpu_busd;
	struct clk *clk_gpu;
	...
	void __iomem *regnoc[4];
	void __iomem *regack[4];
...
/* Maximum number of loops polling the NOC for disabling GPU bridge */
#define KBASE_NOC_ENABLE_DISABLE_MAX_LOOPS	100000
