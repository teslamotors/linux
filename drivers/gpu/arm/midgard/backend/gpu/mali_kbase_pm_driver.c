...
void kbase_pm_enable_noc_snoop(struct kbase_device *kbdev)
{
	int i;
	u32 loops = KBASE_NOC_ENABLE_DISABLE_MAX_LOOPS;

	for (i = 0; i < 4; i++) {
		writel(ENABLE, kbdev->regnoc[i]);
		while ((!(readl(kbdev->regack[i]) & COHERENCY_EN_BIT)) && --loops);
	}
}

void kbase_pm_disable_noc_snoop(struct kbase_device *kbdev)
{
	int i;
	u32 loops = KBASE_NOC_ENABLE_DISABLE_MAX_LOOPS;

	for (i = 0; i < 4; i++) {
		writel(DISABLE, kbdev->regnoc[i]);
		while ((readl(kbdev->regack[i]) & COHERENCY_EN_BIT) && --loops);
	}
}
...
void kbase_pm_cache_snoop_enable(struct kbase_device *kbdev)
{
	if ((kbdev->current_gpu_coherency_mode == COHERENCY_ACE) &&
		...
		if (kbdev->snoop_enable_smc != 0)
			...
			kbase_pm_enable_noc_snoop(kbdev);
#endif /* CONFIG_ARM64 */
...
void kbase_pm_cache_snoop_disable(struct kbase_device *kbdev)
{
	if (kbdev->cci_snoop_enabled) {
		...
		if (kbdev->snoop_disable_smc != 0) {
			...
			mali_cci_flush_l2(kbdev);
			kbase_pm_disable_noc_snoop(kbdev);
		}
#endif /* CONFIG_ARM64 */
