...
static int kbase_set_clock_frequency(struct kbase_device *kbdev, unsigned long freq)
{
	int err = 0;

	if (!kbdev->mout_clk_gpu_switch_user || !kbdev->mout_clk_gpu_busd || !kbdev->clock) {
		dev_err(kbdev->dev, "GPU clock is not available. check clock were setup properly\n");
		return 1;
	}

	err = clk_prepare_enable(kbdev->mout_clk_gpu_switch_user);
	if (err) {
		dev_err(kbdev->dev, "Can not enable switch clock\n");
		return err;
	}
	err = clk_set_parent(kbdev->mout_clk_gpu_busd, kbdev->mout_clk_gpu_switch_user);
	if (err) {
		dev_err(kbdev->dev, "Can not switch clock as parent\n");
		return err;
	}

	err = clk_set_rate(kbdev->clock, freq);
	if (err) {
		dev_err(kbdev->dev, "Can not set clock rate\n");
		return err;
	}

	err = clk_set_parent(kbdev->mout_clk_gpu_busd, kbdev->clock);
	if (err) {
		dev_err(kbdev->dev, "Can not set clock rate\n");
		return err;
	}

	(void)clk_disable_unprepare(kbdev->mout_clk_gpu_switch_user);
...
kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	err = kbase_set_clock_frequency(kbdev, freq);
	if (err) {
