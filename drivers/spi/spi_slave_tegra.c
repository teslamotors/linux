 ...
 * However, it is recommended to keep controller clock 8 times of interface clock
 * to avoid some timing issue.
 * */
...
{
	unsigned long val;
	unsigned int len, unused_fifo_size, status2_reg_val;
	int ret = 0;
	...
	if (tspi->cur_direction & DATA_DIR_TX) {
		...
		unused_fifo_size = ((tspi->tx_dma_req.size/4) >= 32) ? 0 :
									32 - (tspi->tx_dma_req.size/4);

		/* Wait for tx fifo to be fill before starting slink */
		do{
			status2_reg_val = spi_tegra_readl(tspi, SLINK_STATUS2);
		} while (SLINK_TX_FIFO_EMPTY_COUNT(status2_reg_val) > unused_fifo_size);
	}
		    ...
		    struct spi_transfer *t)
{
	...
	command2 &= ~(SLINK_SS_EN_CS(~0) | SLINK_RXEN | SLINK_TXEN | SLINK_LSBFE);
...
static int spi_tegra_transfer(struct spi_device *spi, struct spi_message *m)
{
	...
	if (was_empty) {
		...
		if(t->delay_usecs)
		{
			long wait_jiffies = usecs_to_jiffies(t->delay_usecs)? :1;
			mod_timer(&tspi->transmit_timer, jiffies + wait_jiffies);
		}
	}
...
static void handle_cpu_based_xfer(void *context_data)
{
	...
	if (tspi->tx_status ||  tspi->rx_status ||
		...
		dev_dbg(&tspi->pdev->dev, "%s ERROR bit set 0x%x\n",
...
static irqreturn_t spi_tegra_isr_thread(int irq, void *context_data)
{
	...
	if (tspi->cur_direction & DATA_DIR_TX) {
		if (tspi->tx_status) {
			...
			wait_status = try_wait_for_completion(&tspi->tx_dma_complete);
			if (wait_status == 0) {
				tegra_dma_dequeue(tspi->tx_dma);
	...
	if (tspi->cur_direction & DATA_DIR_RX) {
		if (tspi->rx_status) {
			...
			int try = 100;
			while(!(wait_status = try_wait_for_completion(&tspi->rx_dma_complete)) && try--) {
				udelay(5);
			}
			if (wait_status == 0) {
...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	setup_timer(&tspi->transmit_timer, spi_tegra_complete_with_error, (unsigned long)tspi);
...
/* It will abort a single dma request. */
...
void spi_tegra_abort_transfer(struct spi_device *spi)
{
	...
	struct spi_message *m;
	unsigned long flags;
	unsigned long val, val_write;
	struct spi_transfer *t = tspi->cur;
	unsigned int word_tfr, pending_rx_words;

	/* Abort is not supported for cpu mode presently */
	if (!tspi->is_curr_dma_xfer)
		return;
	...
	/* Rx fifo: In abort case dma will transfer equal or less than the
	 * slink has received. If slink receives more data and dma have not
	 * transferrd from the rx fifo to memory then there may be some more
	 * data available into the fifo. Reading those in pio mode.
	 * Tx fifo: In abort case  dma will transfer more than slink has and
	 * tx fifo holds those, which will get reset after tx-flush/slink reset.
	 * No need to do anything for tx case.
	 */

	if (tspi->cur_direction & DATA_DIR_RX){
		word_tfr = tspi->rx_dma_req.bytes_transferred / 4;
		pending_rx_words =  SLINK_RX_FIFO_FULL_COUNT(
				spi_tegra_readl(tspi, SLINK_STATUS2));
		tspi->rx_dma_req.bytes_transferred +=  pending_rx_words * 4;
		while (pending_rx_words){
			tspi->rx_buf[word_tfr] = spi_tegra_readl(tspi, SLINK_RX_FIFO);
			pending_rx_words--;
			word_tfr++;
		}
		// copy bounce buf to client now
		spi_tegra_copy_spi_rxbuf_to_client_rxbuf(tspi, t);
	}
	...
	list_del(&m->queue);
	m->actual_length = SLINK_BLK_CNT(val);
...
MODULE_ALIAS("m:tegra_spi_slave");
