    ...
    unsigned int cmd, unsigned long arg)
{
    ...
    case NvRmIoctls_Generic:
    {
        ...
        {
	    NvOsIoctlParams params;
	    err = NvOsCopyIn(&params, (void *)arg, sizeof(params));
	    if (err != NvSuccess) {
	      printk( "NvRmIoctls_Generic: failed to get arguments\n");
	      goto fail;
	    }

            err = NvOsCopyOut( ((NvU8 *)params.pBuffer)
                + p.InBufferSize,
	...
	/*
	 * bug 788110: glibc ends up generating a spurious TCGETS ioctl when calling fdopen in
	 * NvOsFopenInternal (core/utils/nvos/linux/nvos_linux.c)
	 * Absorb it silently
	 */
	case TCGETS:
		break;
	// FIXME: power ioctls?
	default:
		printk( "NV: unknown ioctl code 0x%x\n", cmd );
		goto fail;
	}
