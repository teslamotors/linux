...
#error "fix this file for LPAE - assumes too much about 2 level pagetables"
#endif

#if 0
#define __dbg(x...) pr_info(x)
#define PAGEATTR_DBG_ON
#else
#define __dbg(x...) do { } while(0)
#endif
...
static inline pte_t pmd_pfn(pmd_t pmd)
{
	u32 pmdval = pmd_val(pmd);

	/* top 20 bits are phys-addr of PMD, so shift down SECTION_SHIFT and back PAGE_SHIFT
	 * to get back to the value that we should feed to __pte()  */
        return __pte((pmdval >> SECTION_SHIFT) << (SECTION_SHIFT - PAGE_SHIFT));
}
...
phys_addr_t lowmem_limit = 0xfe000000;	/* todo - calc from real value */
...
pte_t *lookup_address(unsigned long address, unsigned int *level)
{
	...
	*level = PG_LEVEL_NONE;
	...
	if (((pmd_val(*pmd) & (PMD_TYPE_SECT | PMD_SECT_SUPER))
		...
		__dbg("pmd is sect: addr=%08x, 2M, pmd_val=%08x\n", address, pmd_val(*pmd));
		*level = PG_LEVEL_2M;
		return (pte_t *)pmd;
	}
...
static void __set_pmd_pte(pmd_t *pmd, unsigned long address, pte_t *pte)
{
	...
	__dbg("%s: pmd %p=%08x,%08x\n", __func__, pmd, pmd[0], pmd[1]);
...
static int split_large_page(pte_t *kpte, unsigned long address)
{
	...
	__dbg("%s: pmd %p(%lx), address 0x%08x, pbase %p (%08x)\n",
	      __func__, kpte, *kpte, address, pbase, __pa(pbase));
	...
	*/
	...
	/* i think we would be ok if we're only dealing with XN pages */
	if ((ref_prot & ~L_PTE_XN) != pgprot_kernel) {
		pr_info("BUG %s: addr=%08lx, ref_prot=%08x, pgprot_kernel=%08x\n",
			__func__, address, ref_prot, pgprot_kernel);
		BUG_ON(1);
	}
	/*
	 ...
	 */
	pfn = pmd_pfn(*kpte);
	__dbg("%s: pfn %08x (%08x), kpte %p (%08x)\n", __func__, pfn, pfn_to_kaddr(pfn), kpte, *kpte);
	for (i = 0; i < PTRS_PER_PTE; i++, pfn += pfninc) {
		if (i < 2)
			__dbg("%s: set_ptex for %p, pfn %08x (%08x), prot %08x,%08x\n",
			      __func__, &pbase[i], pfn, pfn_to_kaddr(pfn), ref_prot, ext_prot);
		      
		set_pte_ext(&pbase[i], pfn_pte(pfn, ref_prot), ext_prot);
#ifdef PAGEATTR_DBG_ON
		if (i < 2) {
			pte_t *pp = &pbase[i];
			__dbg("%s: pbase %p = %08x, arm ent %08x\n",
			      __func__, pp, pp[0], pp[2048/4]);
		}
#endif
	}
	 ...
	 */
	__dbg("%s: about to install new pmd: kpte %p, addr %08x, pbase %p\n",
	      __func__, kpte, address, pbase);
