...
static int set_submit(struct nvhost_channel_userctx *ctx)
{
	...
	/* leave room for ctx switch and 2D serialization wait */
	ctx->num_gathers = 3;
	ctx->pinarray_size = 0;
                                      ...
                                      struct nvhost_get_param_args *args)
{
	...
	int num_unpin;
	int err;
	int nulled_incrs = ctx->hdr.null_kickoff ?
		ctx->hdr.syncpt_incrs : 0;
	...
	if (gather_idx == 3 && ctx->ch->desc->class) {
		gather_idx--;
		ctx->gathers[gather_idx].op1 =
	...
	/* serialize 2D jobs */
	if (ctx->ch->desc->serialize) {
		gather_idx--;
		ctx->gathers[gather_idx].op1 =
			nvhost_opcode_setclass(NV_HOST1X_CLASS_ID,
				NV_CLASS_HOST_WAIT_SYNCPT,
				1);
		ctx->gathers[gather_idx].op2 =
			nvhost_class_host_wait_syncpt(ctx->hdr.syncpt_id,
				nvhost_syncpt_read_max(&ctx->ch->dev->syncpt,
				ctx->hdr.syncpt_id));
	}
...
static int __devinit nvhost_probe(struct platform_device *pdev)
{
	...
	tegra_periph_reset_assert(host->mod.clk[0]);
	mdelay(10);

	clk_enable(host->mod.clk[0]);

	tegra_periph_reset_deassert(host->mod.clk[0]);
	mdelay(10);
