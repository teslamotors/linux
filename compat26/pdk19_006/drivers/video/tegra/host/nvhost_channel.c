...
{
	u32 spl, stop = 0x000000FF, tear = 0x00000010;
	...
	if (ch->refcount == 1) {
		...
		#ifdef CONFIG_TEGRA_GRHOST_VI_RECOVERY
		if (!strcmp(ch->mod.name, "vi")) {
			spl = readl(ch->dev->sync_aperture + HOST1X_SYNC_CMDPROC_STOP);
			writel(stop, ch->dev->sync_aperture + HOST1X_SYNC_CMDPROC_STOP);
			writel(tear, ch->dev->sync_aperture + HOST1X_SYNC_CH_TEARDOWN);
			writel(spl, ch->dev->sync_aperture + HOST1X_SYNC_CMDPROC_STOP);
		}
		#endif
	}
	ch->refcount--;
			...
			u32 syncpt_id, u32 syncpt_val, int num_nulled_incrs)
{
	...
	if (waitchk != waitchk_end) {
		err = nvhost_syncpt_wait_check(user_nvmap,
				&ch->dev->syncpt, waitchk_mask,
				waitchk, waitchk_end);
		if (err) {
			dev_warn(&ch->dev->pdev->dev,
