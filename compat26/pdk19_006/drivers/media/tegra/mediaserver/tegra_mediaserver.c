...
#define MAX_HWA_AUDIO_INSTANCES 1
...
static unsigned int nHWAAudioDecCount;
				    ...
				    unsigned long arg)
{
	...
	case TEGRA_MEDIASERVER_IOCTL_SET_HWA_AUDIODEC_COUNT:
	{
		unsigned int in;
		e = copy_from_user(&in, (void __user *)arg, sizeof(in));
		CHECK_CONDITION(e, copy_fail);

		if (in > MAX_HWA_AUDIO_INSTANCES)
		{
		    e = -EINVAL;
		    goto fail;
		}
		nHWAAudioDecCount = in;
	}
	break;

	case TEGRA_MEDIASERVER_IOCTL_GET_HWA_AUDIODEC_COUNT:
	{
		e = copy_to_user((void __user *)arg, &nHWAAudioDecCount, sizeof(nHWAAudioDecCount));
		CHECK_CONDITION(e, copy_fail);
	}
	break;
	default:
