...
static tegra_iovmm_addr_t iovmm_align_up(int align_bits,
...
static tegra_iovmm_addr_t iovmm_align_down(int align_bits,
	tegra_iovmm_addr_t addr)
{
	addr &= ~((1 << align_bits) - 1);
	return addr;
...
#define SIMALIGN(b, a)	(((b)->start%(a)) ? ((a)-((b)->start%(a))) : 0)
	...
	BUG_ON(!size);
	size = iovmm_align_up(domain->dev->pgsize_bits, size);
	align = iovmm_align_up(domain->dev->align, align);
	for (;;) {
		spin_lock(&domain->block_lock);
	...
	if (DO_SPLIT(simalign)) {
		...
		b = best;
		best = iovmm_split_free_block(domain, b, simalign, rem);
		if (best)
			simalign = 0;
		else
			best = b;
	...
	if (DO_SPLIT((best->start + best->length) - iovmm_end(best))) {
		...
		/* Split off excess */
		(void)iovmm_split_free_block(domain, best, size + simalign, rem);
	}

	iovmm_block_splitting = 0;
	spin_unlock(&domain->block_lock);

	BUG_ON(best->vm_area.iovm_start % align);
	...
	size_t align, unsigned long iovm_start)
{
	struct rb_node *n;
	struct tegra_iovmm_block *b, *best;
	size_t addr_align = (size_t)max(align, (size_t)(1 << domain->dev->align));

	BUG_ON(iovm_start % addr_align);
	BUG_ON(!size);
	...
	if (DO_SPLIT(best->start + best->length - iovmm_end(best))) {
		...
		(void)iovmm_split_free_block(domain, best,
			(iovmm_start(best) - best->start + size), rem);
	}
fail:
	iovmm_block_splitting = 0;
	spin_unlock(&domain->block_lock);
	...
	tegra_iovmm_addr_t end)
{
	...
	b->start  = iovmm_align_up(dev->pgsize_bits, start);
	b->length = iovmm_align_down(dev->pgsize_bits, end) - b->start;
