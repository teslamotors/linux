		  ...
		  unsigned int nr, const unsigned long *ids)
{
	...
	if (ret) {
		/* fix off-by-one from the above loop. */
		nr--;
		nvmap_ref_unlock(client);
		goto out;
	}
				 ...
				 int nr, struct nvmap_handle *gather)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (patch->heap_pgalloc) {
			...
			if (page >= PAGE_ALIGN(patch->size) >> PAGE_SHIFT) {
				pr_warning_ratelimited(
					 "nvmap_reloc_pin_array: page %x %x\n",
					 page, patch->size);
				nvmap_handle_put(patch);
				nvmap_free_pte(client->dev, pte);
				return -EINVAL;
			}
		...
		if ((phys & ~PAGE_MASK) > PAGE_SIZE - sizeof(uint32_t)) {
			pr_warning_ratelimited(
				"nvmap_reloc_pin_array: phys %x\n", phys);
			nvmap_handle_put(patch);
			nvmap_free_pte(client->dev, pte);
			return -EINVAL;
		}
		    ...
		    struct nvmap_handle **unique_arr)
{
	...
	if (WARN_ON(ret)) {
		nvmap_unpin_handles(client, unique_arr, count);
		return ret;
	} else {
		for (i = 0; i < count; i++) {
...
void *nvmap_mmap(struct nvmap_handle_ref *ref)
{
	...
	v = compat_alloc_vm_area(adj_size);
	if (!v) {
