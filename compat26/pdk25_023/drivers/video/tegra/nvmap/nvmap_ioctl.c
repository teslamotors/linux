...
int nvmap_ioctl_pinop(struct file *filp, bool is_pin, void __user *arg)
{
	...
	if (op.count > 1) {
		...
		if (op.count > ARRAY_SIZE(on_stack)) {
			if (op.count > UINT_MAX / sizeof(*refs))
				return -EINVAL;
...
int nvmap_map_into_caller_ptr(struct file *filp, void __user *arg)
{
	...
	if (op.offset > UINT_MAX - op.length || (op.offset + op.length) > h->size) {
		err = -EADDRNOTAVAIL;
		goto out;
	}
...
int nvmap_ioctl_cache_maint(struct file *filp, void __user *arg)
{
	...
	if (!vma || !is_nvmap_vma(vma) ||
	    op.len > ULONG_MAX - (unsigned long)op.addr ||
			 ...
			 unsigned long count)
{
	...
	while (count--) {
		if (h_offs > UINT_MAX - elem_size ||
				h_offs + elem_size > h->size) {
		...
		if (sys_addr > ULONG_MAX - sys_stride ||
				h_offs > ULONG_MAX - h_stride) {
			ret = -EINVAL;
			break;
		}
