...
int tegra_edid_extract_monspecs(struct tegra_edid *edid,
		struct tegra_dc_edid *edid_data, struct fb_monspecs *specs)
{
	int i;
	int j;
	int ret;
	...
	memcpy(data, edid_data->buf, edid_data->len);
...
int tegra_edid_get_monspecs(struct tegra_edid *edid, struct fb_monspecs *specs)
{
	...
	edid_data = vmalloc(SZ_32K + sizeof(struct tegra_dc_edid));
	if (!edid_data)
		return -ENOMEM;

	buf = edid_data->buf;

	ret = tegra_edid_read_block(edid, 0, buf);
	if (ret)
		goto done;

	extension_blocks = buf[0x7e];

	for (i = 1; i <= extension_blocks; i++) {
		ret = tegra_edid_read_block(edid, i, buf + i * 128);
		if (ret < 0)
			break;
	}
	...
	ret = tegra_edid_extract_monspecs(edid, edid_data, specs);
...
struct tegra_edid *tegra_edid_create(int bus)
{
	...
	if (!adapter) {
		pr_err("can't get adapter for bus %d\n", bus);
		err = -EBUSY;
		goto free_edid;
	}
