...
static int netdev_wait __initdata = 0;

static int __init netdevwait_setup(char *str)
{
	if (*str)
		return 0;
	netdev_wait = 1;
...
__setup("netdevwait", netdevwait_setup);
...
static int __init wait_for_devices(int net_dev_wait)
{
	int i;

	msleep(CONF_PRE_OPEN);
	for (i = 0; i < DEVICE_WAIT_MAX || net_dev_wait; i++) {
		struct net_device *dev;
		int found = 0;
...
static int __init ip_auto_config(void)
{
 ...
 try_try_again:
	...
	err = wait_for_devices(netdev_wait);
	if (err)
		return err;
